...
class LandmarkIsomap(sklearn.manifold.Isomap):
    ...
    def _fit_transform(self, X):
        ...
        kng = neighbors.kneighbors_graph(self.nbrs_, self.n_neighbors,
                                         mode='distance', n_jobs=self.n_jobs)

        # This function doesn't accept non-square matrices,
        # forcing us to calculate all shortest-paths and
        # not only SP_{landmarks x samples}).
        ...
        self.landmarks_dist_matrix_ = self.dist_matrix_[self.landmarks_, :]
        self.landmarks_dist_matrix_ = self.landmarks_dist_matrix_[:, self.landmarks_]
        G = self.landmarks_dist_matrix_ ** 2
        G *= -0.5
        ...
        self.nbrs_.fit(X[self.landmarks_, :])
    ...
    def transform(self, X):
        ...
        for i in range(X.shape[0]):
            G_X[i] = np.min(self.landmarks_dist_matrix_[indices[i]] +
                            distances[i][:, None], 0)
