...
\subsubsection{Evaluating Reductions}

Isomap's aim to ``unfold" the data set and, of course, it might distort dissimilarities between samples in the process. Hence drastically increasing Kruskal's stress. Clearly, this measure is unfittable for evaluating Isomap. As proposed by L. Shi and J. Gu, Kruskal's stress can be easily updated to match Isomap nature. That is, to preserve dissimilarities within neighborhoods: \cite{shi2012fast}
...
The Dermatology data set, composed by 366 samples and 35 features. Here, the target is to correctly diagnosticate the erythemato-squamous diseases.
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|c|c|c|}
		\hline
		& \textbf{Orig. d.} & \textbf{$\mathbb{R}^{20}$} & \textbf{$\mathbb{R}^{10}$} & \textbf{$\mathbb{R}^3$} & \textbf{$\mathbb{R}^2$} \\\hline
...
The Results in the table above are quite impressive: although visualization has barely improved, a 7129-dimensional space was reduced to a 10-dimensional one with only 10\% of accuracy loss.

\subsubsection{The WDBC Data Set}

The Wisconsin Diagnostic Breast Cancer (WDBC) data set, containing 596 samples and 32 features computed from a breast mass. WDBC has benign 357 samples and 212 malignant. During this experiment, the first feature was disconsidered, as it represents the identification numbers of the samples.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{img/experiments/iso_wdbc}
	\captionsetup{justification=centering}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|c|c|c|}
		\hline
		& \textbf{$\mathbb{R}^{30}$} & \textbf{$\mathbb{R}^{20}$} & \textbf{$\mathbb{R}^{10}$} & \textbf{$\mathbb{R}^{3}$} & \textbf{$\mathbb{R}^{2}$} \\\hline
...
\subsubsection{Diabetes Data Set}

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{img/experiments/iso_diabetes}
	\captionsetup{justification=centering}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|c|c|c|}
		\hline
		& \textbf{Original data ($\mathbb{R}^{8}$}) & \textbf{$\mathbb{R}^{6}$} & \textbf{$\mathbb{R}^{4}$} & \textbf{$\mathbb{R}^{2}$} & \textbf{$\mathbb{R}$} \\\hline
