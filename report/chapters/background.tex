...
\footnotetext{Retrieved from \href{https://archive.ics.uci.edu/ml/datasets/Iris}{archive.ics.uci.edu}}
...
\footnotetext{Retrieved from \href{https://archive.ics.uci.edu/ml/datasets/ILPD+(Indian+Liver+Patient+Dataset)}{archive.ics.uci.edu}}
...
\footnotetext{Available at: \href{http://mldata.org/repository/data/viewslug/leukemia/}{mldata.org}.}
...
\footnotetext{Retrieved from \href{Weisstein, Eric W. "Torus." From MathWorld--A Wolfram Web Resource. http://mathworld.wolfram.com/Torus.html}{mathworld.wolfram.com/Torus}}
...
\footnotetext{Retrieved from \href{https://en.wikipedia.org/wiki/Manifold}{wiki/Manifold/Charts}}
...
\footnotetext{Source: \href{http://people.brunel.ac.uk/~mastjjb/jeb/or/graph.html}{brunel.ac.uk/~mastjjb}.}
...
\begin{description}
	...
	\footnotetext{Retrieved from \href{https://github.com/Comp-UFSCar/graph-theory}{gihut.com/Comp-UFSCar/graph-theory}.}
...
Contained in the supervised learning category, Support Vector Machines (SVM) are powerful tools, often used in classification and regression problems.
...
More elaborately, given any linearly separable data set $\vect X$ containing samples from two distinguished classes $\{-1, +1\}$, consider the vector $w$ and the hyperplane $d = \{x \mid w \cdot x + b = 0\}$ (represented in fig. \ref{fig:svmmargin} by the contiguous line), a decision rule $f$ is defined as:
\begin{align*}
	f(x_i) = \begin{cases}
...
To deal with non-separable data sets, it is possible to introduce the variables $\xi_i$ to the optimizing equation and decision rule \cite{wessvmdef}, which represent a trade-off between maximum margin/distance of the misclassified samples from the decision boundary.
$$\min \frac{1}{2} \|w\|^2 + C \sum_{1}^{m}\xi_i \text{, constrained to:}$$
$$f(x_i) = y_i(w \cdot x_i +b) \geq 1- \xi_i, \xi_i \geq 0$$
...
Between many different kernels, two often used are the RBF: $\exp(-\frac{\|x -x'\|^2}{2\sigma^2})$ and the Polynomial: $(u^\top v + c)^{d}$ \cite{svmkernels}.
